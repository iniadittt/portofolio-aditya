...
export default function Spotify({ handlerSetPopup, maximize }: { handlerSetPopup: SpotifyProps["handlerSetPopup"]; maximize: boolean }) {
	return (
			...
			<div className={`relative w-full h-full bg-white shadow-xl ${!maximize && "rounded-lg"}`}>
				<div className={`absolute border-b bg-white w-full flex items-center gap-2 py-2 px-3 ${!maximize && "rounded-t-lg"}`}>
					<button
						...
						onClick={(event: React.MouseEvent) => handlerSetPopup(event, "spotify", "open")}
					>
						&nbsp;
					</button>
					<button
						className={`cursor-pointer w-3 h-3 rounded-full ${maximize ? "bg-yellow-500" : "bg-orange-500"}`}
						onClick={(event: React.MouseEvent) => handlerSetPopup(event, "spotify", "maximize")}
					>
						&nbsp;
					</button>
				</div>
				<div className={`bg-white w-full h-full pt-10 ${!maximize && "rounded-lg"} p-3`}>
					<div className="bg-blue-200">SPOTIFY.</div>
				</div>
			</div>
		</div>
	);
